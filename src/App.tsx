...
import type { AtlasSceneHandle, CameraMode, Landmark, SceneStatus, TimeOfDay } from './types'
...
export default function App() {
  ...
  function landmarkCard(item: Landmark) {
    ...
    return (
      <article className={`landmark-card ${noteCollapsed ? 'collapsed' : ''}`} aria-label={`${item.name} field notes`}>
        ...
        <details className="evidence-note"><summary>Reconstruction note</summary><p>{item.caveat}</p><p>Map position is approximate. All 3D models are original interpretive miniatures, with exaggerated height and detail for legibility.</p></details>
  ...
  return (
    <div className={`atlas-app ${immersive ? 'immersive' : ''}`}>
      ...
      <main className="atlas-main">
        ...
        <div className="world-viewport" aria-label="Interactive Hyderabad city scene">
          ...
          {sceneStatus === 'loading' && <div className="scene-message" role="status"><span className="loading-orbit" />Assembling the Deccan landscape…</div>}
          {sceneStatus === 'unavailable' && <div className="scene-fallback"><span className="eyebrow">MAP MODE</span><h2>A different way to explore.</h2><p>3D rendering is unavailable in this browser. The timeline, landmark map, and source-linked history remain fully accessible.</p><Minimap large landmarks={LANDMARKS} year={era.year} selectedId={selectedId} onSelect={focusPlace} /></div>}
        </div>

        <div className="map-caption"><span className="live-dot" />{sceneStatus === 'unavailable' ? '2D FIELD GUIDE' : 'INTERPRETIVE 3D ATLAS'}<span>17.4° N / 78.5° E</span></div>
        ...
        <aside id="chapter-panel" className={`chapter-panel ${mobilePanel ? 'mobile-open' : ''}`} tabIndex={-1}>
          ...
          <div className="chapter-scroll">
            ...
            {tab === 'story' ? <>
            ...
            </> : tab === 'life' ? <CityLifePanel story={life} districts={districts} explored={exploredDistricts} onVisit={visitDistrict} onSources={() => setSourcesOpen(true)} />
              ...
              <label className="search-label" htmlFor="place-search">Find a place in {era.year}</label>
              ...
              <div className="places-list">
                ...
                {!visiblePlaces.length && <p className="empty-note">No places match in this chapter. Try another era or search.</p>}
              </div>
        ...
        <div className="scene-toolbar">
          <div className="tool-group" aria-label="Camera navigation">
            ...
            <button className={cameraMode === 'walk' ? 'tool-button active' : 'tool-button'} aria-pressed={cameraMode === 'walk'} onClick={() => setCameraMode('walk')}><Icon name="walk" />Street view</button>
          </div>
          ...
          {layersOpen && <div className="layers-panel">
            ...
            <label><input type="checkbox" checked={showCity} onChange={(e) => setShowCity(e.target.checked)} />Interpretive city fabric</label>
            <label><input type="checkbox" checked={showRoads} onChange={(e) => setShowRoads(e.target.checked)} />Schematic streets</label>
            <p>No survey data or historical flood boundary is implied.</p>
          </div>}
        </div>

        {selected && <div className={`field-note-panel ${noteCollapsed ? 'is-collapsed' : ''}`}>{landmarkCard(selected)}</div>}
        {cameraMode === 'walk' && activeDistrict && <div className="district-hud"><span className="eyebrow">{era.year} · ON THE STREET</span><strong>{activeDistrict.name}</strong><span>Reconstructed lanes · drag to look · WASD to move</span></div>}
        ...
        <div className={`map-tools ${selected ? 'with-selection' : ''}`}>
          <div className="lighting-control" aria-label="Scene lighting">
          ...
          <div className="navigation-row">
            <div className="mini-map-card"><Minimap landmarks={LANDMARKS} selectedId={selectedId} year={era.year} onSelect={focusPlace} /><span className="mini-caption">ORIENTATION · MARKERS OFFSET FOR SELECTION</span></div>
            <div className="camera-buttons">
              <button aria-label="Reset aerial view" title="Reset aerial view" onClick={() => { setCameraMode('orbit'); scene.current?.home(); setSelectedId(null); setPinPlace(false); setTourIndex(null) }}><Icon name="home" /></button>
              <button aria-label="Show metropolitan overview" title="Show metropolitan overview" onClick={() => { setCameraMode('orbit'); scene.current?.region(); setSelectedId(null); setPinPlace(false); setTourIndex(null) }}><Icon name="compass" /></button>
        ...
        <div className="navigation-hint">{cameraMode === 'orbit' ? 'DRAG TO ORBIT · RIGHT-DRAG TO PAN · SCROLL TO ZOOM' : 'W A S D TO EXPLORE · DRAG TO LOOK · SHIFT TO MOVE FASTER'}<span>{cameraMode === 'walk' ? 'Free camera · no collision simulation' : 'Touch: drag and pinch'}</span></div>
      ...
      <dialog ref={sourcesDialog} className="research-dialog" onCancel={() => setSourcesOpen(false)} onClose={() => setSourcesOpen(false)}>
        ...
        <div className="evidence-grid">
          ...
          <div><span className="interpretive-dot" /><h3>Interpretive</h3><p>Building shapes and heights, population density, roads, terrain and vegetation. These are visual storytelling, not period cadastral records.</p></div>
        ...
        <p className="research-footnote">The roughly 48 × 50 km metropolitan envelope contains 25 interpreted neighbourhood zones, not a complete building inventory or HMDA boundary. Modern district names are orientation labels. Ground outside monument models is a flat traversal surface, not a measured elevation survey. All models are original geometry. Research assembled September 2026.</p>
      </dialog>
    </div>
  )
}
